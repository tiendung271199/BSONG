...
<div class="content_resize">
	<div class="mainbar">
		...
		<div class="article">
			...
			<p class="infopost">Ngày đăng: <%=ngayDang %> - Lượt xem: <%=objSong.getCounter() %> <a href="<%=urlSlug %>" class="com"><span><%=i++ %></span></a></p>
			<div class="clr"></div>
			<div class="img">
				<%
		...
		<p class="pages">
			...
			<%
				...
				if (pages > 1) {
					...
					if (back == 1) {
		    %>
		    <a href="<%=request.getContextPath() %>/index">&laquo;</a>
		    <%
					} else {
		    %>
		    <a href="<%=request.getContextPath() %>/trang-<%=back %>.html">&laquo;</a>
		    <%
					}
				}
				// Button Number page
            ...
            <%
            			...
            			if (j == 1) {
            %>
            <a href="<%=request.getContextPath() %>/index"><%=j %></a>
            <%
            			} else {
            %>
            <a href="<%=request.getContextPath() %>/trang-<%=j %>.html"><%=j %></a>
            <%
            			}
            		}
                }
                ...
                if (total % 2 != 0) {
                    if (pages != (total / 4) + 1) {
                        next = pages + 1;
             %>
             <a href="<%=request.getContextPath() %>/trang-<%=next %>.html">&raquo;</a>
             <%
                    }
                } else {
                    //Nếu total chẵn nhỏ hơn fullpage và không fullPage thì thêm 1
