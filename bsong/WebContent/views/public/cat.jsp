...
<div class="content_resize">
	<div class="mainbar">
		<%
			...
			if (request.getAttribute("listsong") != null) {
				...
				if (listSong.size() > 0) {
					...
					catId = song.getCat().getId();
					tenCat = StringUtil.makeSlug(song.getCat().getName());
		%>
		<div class="article">
			<h1><%=song.getCat().getName() %></h1>
		</div>
		<%
		...
		<div class="article">
			...
			<p class="infopost">
				Ngày đăng: <%=ngayDang %> - Lượt xem: <%=objSong.getCounter() %> <a href="<%=urlSlug %>" class="com"><span><%=i++ %></span></a>
			</p>
			<div class="clr"></div>
			<div class="img">
				<%
		...
		<p class="infopost">Không có bài hát nào!</p>
		<%
				}
			}
			}
		...
		<p class="pages">
			...
			<%
				...
				if (pages > 1) {
					...
					if (back == 1) {
		    %>
		    <a href="<%=request.getContextPath() %>/danh-muc/<%=tenCat %>-<%=catId %>.html">&laquo;</a>
		    <%
					} else {
		    %>
		    <a href="<%=request.getContextPath() %>/trang-<%=back %>/<%=tenCat %>-<%=catId %>.html">&laquo;</a>
		    <%
					}
				}
				// Button Number page
            ...
            <%
            			...
            			if (j == 1) {
            %>
            <a href="<%=request.getContextPath() %>/danh-muc/<%=tenCat %>-<%=catId %>.html"><%=j %></a>
            <%
            			} else {
            %>
            <a href="<%=request.getContextPath() %>/trang-<%=j %>/<%=tenCat %>-<%=catId %>.html"><%=j %></a>
            <%
            			}
            		}
                }
                ...
                if (total % 2 != 0) {
                    if (pages != (total / 4) + 1) {
                        next = pages + 1;
             %>
             <a href="<%=request.getContextPath() %>/trang-<%=next %>/<%=tenCat %>-<%=catId %>.html">&raquo;</a>
             <%
                    }
                } else {
                    //Nếu total chẵn nhỏ hơn fullpage và không fullPage thì thêm 1
