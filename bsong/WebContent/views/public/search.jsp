...
<div class="content_resize">
	<div class="mainbar">
		...
		<p class="infopost">Không tìm thấy!</p>
		<%
				}
			}
			if (check == 0) {
		...
		<p class="pages">
			...
			<%
				...
				if (pages > 1) {
					int back = pages - 1;
		    %>
		    <a href="<%=request.getContextPath() %>/tim-kiem/trang-<%=back %>/<%=searchContent %>.html">&laquo;</a>
		    <%
				}
				// Button Number page
				int loop = 0, num = 0;
            ...
            %>
            <a href="<%=request.getContextPath() %>/tim-kiem/trang-<%=j %>/<%=searchContent %>.html"><%=j %></a>
            <%
            		}
                }
           		//Button Next
                ...
                if (total % 2 != 0) {
                    if (pages != (total / 4) + 1) {
                        next = pages + 1;
             %>
             <a href="<%=request.getContextPath() %>/tim-kiem/trang-<%=next %>/<%=searchContent %>.html">&raquo;</a>
             <%
                    }
                } else {
                    //Nếu total chẵn nhỏ hơn fullpage và không fullPage thì thêm 1
